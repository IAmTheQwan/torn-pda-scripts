...
Each watched Auction House item shows its saved bid and a separate **Update** control. One tap fully loads that item's saved Torn page, matches the visible listing by identity/stats/bonuses, and refreshes the local watch snapshot. CAF reports whether the bid changed, was confirmed unchanged, or could not be read; an unreadable live bid never overwrites the saved bid.
