...
class BmgDatabaseTests(unittest.TestCase):
    ...
    def test_torn_userscript_requires_one_direct_press_per_football_game(self) -> None:
        ...
        self.assertIn("Clear local data", script)
        self.assertIn("GUARANTEED-MONEY CHECK", script)
        self.assertIn("ANOMALIES: waiting for a game capture", script)
        ...
        self.assertIn("It is saved only in this userscript manager's private device storage", script)
        self.assertIn("@grant        GM_xmlhttpRequest", script)
        self.assertIn("@grant        GM_getValue", script)
        self.assertIn("@grant        GM_setValue", script)
        self.assertIn("@grant        GM_deleteValue", script)
        ...
        self.assertIn("GITHUB_TOKEN_STORAGE_KEY", script)
        self.assertIn("PANEL_COLLAPSED_STORAGE_KEY", script)
        self.assertIn("panelCollapsed = privateSetting(PANEL_COLLAPSED_STORAGE_KEY, true) !== false", script)
        self.assertIn("async function clearLocalOutbox()", script)
        self.assertIn("transaction.objectStore(STORE_NAME).clear()", script)
        self.assertIn("transaction.objectStore(DELIVERY_STORE_NAME).clear()", script)
        self.assertIn("deletePrivateSetting(GITHUB_TOKEN_STORAGE_KEY)", script)
        self.assertIn("deletePrivateSetting(PANEL_COLLAPSED_STORAGE_KEY)", script)
        self.assertIn("Already uploaded GitHub files will not be deleted.", script)
        self.assertIn("function analyzeEventMarketMath(event)", script)
        self.assertIn("function exhaustiveMarketSelections(market, event)", script)
        self.assertIn("function dutchCandidate(label, legs, source)", script)
        self.assertIn("event.market_math = analyzeEventMarketMath(event)", script)
        self.assertIn("schema_version: 'bmg.market-math.v1'", script)
        self.assertIn("GUARANTEED-MONEY CANDIDATE", script)
        ...
        self.assertIn("It is wager state, not part of the team/line identity.", script)
    ...
    def test_initial_bankroll_and_risk_limits(self) -> None:
        ...
        self.assertEqual(bmg.STARTING_BANKROLL, bmg.risk_eligible_bankroll(row))
        self.assertEqual(
            {
                "reserve": 40_156_081,
